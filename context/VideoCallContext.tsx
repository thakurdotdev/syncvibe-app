...
// Create context
export const VideoCallContext = createContext<VideoCallContextType | undefined>(
  undefined,
);
...
export const VideoCallProvider: React.FC<{ children: React.ReactNode }> = ({
    ...
    const checkStats = async () => {
      try {
        ...
        const avgRoundTripTime =
          samplesCount > 0 ? roundTripTime / samplesCount : 0;
  ...
  const adjustMediaQuality = async (
  ...
  const initializePeerConnection = useCallback(
    async (remotePeerId: number) => {
      ...
      try {
        logWithTimestamp(
          `Initializing peer connection with remote ID: ${remotePeerId}`,
        );
        ...
        peer.addEventListener("iceconnectionstatechange", () => {
          ...
          if (peer.iceConnectionState === "failed") {
            ...
            logWithTimestamp(
              "ICE connection disconnected, waiting before restart",
            );
            setTimeout(() => {
  ...
  const flipCamera = useCallback(() => {
    ...
    logWithTimestamp(
      `Flipping camera from ${cameraType} to ${
    ...
    const handleCallAccepted = async (data: {
      ...
      try {
        ...
        if (
          ...
          logWithTimestamp(
            "Cannot set remote description - peer connection is closed or null",
          );
        }
      } catch (error) {
    ...
    const handleIceCandidate = async (data: { candidate: RTCIceCandidate }) => {
      try {
        if (
          ...
          logWithTimestamp(
            "Skipped ICE candidate - no remote description or connection closed",
          );
        }
      } catch (error) {
